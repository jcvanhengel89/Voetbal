...
function App() {
  ...
  const [archive, setArchive] = useState(() => {
    try { return JSON.parse(localStorage.getItem(storageKey('archive')) || '[]'); } catch { return []; }
  });
  const [opponent, setOpponent] = useState(() => {
    try { return JSON.parse(localStorage.getItem(storageKey('opponent')) || JSON.stringify({ name: 'Tegenstander', colorId: 'blue' })); } catch { return { name: 'Tegenstander', colorId: 'blue' }; }
  });
  ...
  const currentPositions = FORMATIONS[formation]?.positions || FORMATIONS['6v6'].positions;

  const handleOpponentNameChange = useCallback((n) => setOpponent((o) => ({ ...o, name: n })), []);
  const handleOpponentColorChange = useCallback((c) => setOpponent((o) => ({ ...o, colorId: c })), []);
  ...
  const handleFormationChange = useCallback((newFormat) => {
    ...
    setPlayers((curr) => curr.map((p) => (p.pos && !newPositions[p.pos] ? { ...p, pos: null } : p)));
  ...
  const performReset = useCallback(() => {
    ...
    setPlayers((curr) => curr.map((p) => ({ ...p, goals: 0, benchTime: 0, playTime: 0, benchCount: 0, posStats: { k: 0, def: 0, mid: 0, att: 0 } })));
    setSelectedId(null);
            ...
            <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide min-h-[80px]">
              {benchPlayers.length > 0 ? benchPlayers.map((p) => (
                <div key={p.id} onClick={() => handleInteraction(p.id)} className="flex-shrink-0 cursor-pointer">
                  <PlayerToken name={p.name} initials={p.name ? p.name.substring(0, 2).toUpperCase() : '??'} goals={p.goals} isSelected={p.id === selectedId} benchTime={p.benchTime} showBenchTimer />
                </div>
