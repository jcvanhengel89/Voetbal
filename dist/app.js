...
function eventDescription(e){if(e.type==='formation')return `<strong>Formatie gewijzigd</strong><small>${e.beforeFormation} → ${e.formation}</small>`;if(e.type==='goal')return `<strong>Doelpunt ${e.side==='us'?'Nieuwerkerk':esc(state.match.opponent||'tegenstander')}</strong>`;const out=e.before.filter(id=>id&&!e.lineup.includes(id)),incoming=e.lineup.filter(id=>id&&!e.before.includes(id));return `<strong>${out.length||incoming.length?'Wissel':'Posities gewisseld'}</strong><small>${incoming.length?'In: '+incoming.map(id=>esc(name(id))).join(', '):''}${incoming.length&&out.length?' · ':''}${out.length?'Uit: '+out.map(id=>esc(name(id))).join(', '):''}</small>`;}
...
function goal(side){const m=state.match;if(m.status==='ready')return;changeGame(state,side==='us'?'doelpunt Nieuwerkerk':'doelpunt tegenstander',()=>m.events.push({id:uid(),type:'goal',side,at:elapsed(m)}));syncHistory();update();toast(side==='us'?'Doelpunt Nieuwerkerk!':'Doelpunt tegenstander');}
...
function syncHistory(){
 ...
 if(entry)entry.match=structuredClone(state.match);
}
...
const composeSummary=m=>(matchSummary(m)+(m.notes?'\n\n'+m.notes:'')).slice(0,10000);
let summaryTargetId='';
