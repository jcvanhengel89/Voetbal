...
function matchView(){
 ...
 return `<div class="single match-screen">
  ...
  ${!ended?`<section class="card quick-bench"><div class="card-head"><h2>Wisselbank</h2><span class="small muted">Minste speeltijd eerst</span></div><div class="bench">${benchButtons()||`<span class="muted small">${!state.players.length?'Voeg spelers toe via Mijn team.':'Geen bankspelers aanwezig.'}</span>`}</div>${m.status==='ready'?'<p class="hint">Stel je team op via het tabblad Opstelling.</p>':''}</section>`:''}
