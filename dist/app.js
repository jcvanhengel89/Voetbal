...
function suggestionButtons(){return state.match.status==='live'?`<div class="row suggestion-buttons">${bench().length?'<button data-action="suggest-subs">Wisselvoorstel</button>':''}${needsKeeperChange(state.match)?'<button data-action="suggest-keeper">Keeper tweede helft</button>':''}</div>`:'';}
...
function lineupCard(showBench=true){return `<section class="card"><div class="card-head"><h2>Op het veld</h2><span class="pill">${state.match.lineup.filter(Boolean).length} / 6</span></div>${formationSelect()}${pitch()}<button class="wide presentation-button" data-action="show-lineup">${expandIcon()} Opstelling tonen</button>${showBench?`<div class="subtle-row"><h3>Wisselbank</h3><span class="small muted">${bench().length} spelers</span></div><div class="bench">${benchButtons()}</div>${suggestionButtons()}`:''}<p class="hint">${state.players.length?'Tik op een positie of bankspeler om te wisselen.':'Voeg je spelers toe bij Mijn team en tik op een positie.'}</p></section>`;}
function matchView(){
 const m=state.match,s=score(m),live=m.status==='live',ended=m.status==='ended',last=state.undoHistory.at(-1);
 const team=(side,label)=>`<div class="score-team ${side==='us'?'our-team':''}"><span class="team-name">${esc(label)}</span><strong class="score-value">${s[side]}</strong><div class="score-controls"><button class="goal-minus" data-action="minus-${side}" aria-label="Doelpunt verwijderen voor ${esc(label)}" ${s[side]===0?'disabled':''}>−</button><button class="goal ${side==='us'?'us':''}" data-action="goal-${side}" aria-label="Doelpunt voor ${esc(label)}" ${!live&&!ended?'disabled':''}>+ Goal</button></div></div>`;
 return `<div class="single match-screen">
  <div class="heading"><div><p class="eyebrow">Aan de zijlijn</p><h1>Wedstrijddag</h1></div>${!ended?'<button data-action="edit-match" class="ghost">Instellen</button>':''}</div>
  <section class="card scoreboard">
   <div class="card-head"><span class="small muted">${esc(dateLabel(m))} · ${m.home?'Thuis':'Uit'}</span><span class="pill">${ended?'Afgelopen':live?'Bezig':'Voor de aftrap'}</span></div>
   <div class="score-line">${matchSides(m).map(x=>team(x.side,x.name)).join('<span class="separator">:</span>')}</div>
   <div class="timer"><div class="clock" id="clock">00:00</div><p class="timer-label" id="timer-label"></p>
    <div class="row">${ended?'<button class="primary" data-action="new-match">Nieuwe wedstrijd</button>':`<button class="primary" data-action="clock">${m.startedAt!==null?'Ⅱ Pauze':m.status==='ready'?'▶ Aftrap':'▶ Hervatten'}</button>${live?m.half===1?'<button class="ghost" data-action="second-half">2e helft</button>':'<button class="ghost" data-action="finish">Afronden</button>':''}`}</div>
   </div>
  </section>
  ${(last||live&&m.half===1)?`<div class="match-tools">${last?`<button class="text-button" data-action="undo">Herstel: ${esc(last.label)}</button>`:''}${live&&m.half===1?'<button class="text-button" data-action="finish">Wedstrijd afronden</button>':''}</div>`:''}
  <div id="match-reminders" aria-live="polite"></div>
  ${!ended?`<section class="card quick-bench"><div class="card-head"><h2>Wisselbank</h2><span class="small muted">Minste speeltijd eerst</span></div><div class="bench">${benchButtons()||`<span class="muted small">${!state.players.length?'Voeg spelers toe via Mijn team.':'Geen bankspelers aanwezig.'}</span>`}</div>${suggestionButtons()}${m.status==='ready'?'<p class="hint">Stel je team op via het tabblad Opstelling.</p>':''}</section>`:''}
  <details class="card match-report" ${ended?'open':''}><summary>Verslag & samenvatting</summary>
   <button class="wide summary-button" data-action="summary">Samenvatting voor ouders</button>
   <label class="stack">Notities<textarea data-match-notes maxlength="10000" rows="3" placeholder="Mooie momenten of aandachtspunten…">${esc(m.notes)}</textarea></label><p class="hint">Wordt automatisch bewaard voor je verslag.</p>
   <details><summary>Wedstrijdverloop · ${m.events.length} acties</summary>${timeline()}</details>
  </details>
  ${ended?backupNotice():''}
  <p class="offline-note" id="connection"></p>
 </div>`;
}
...
function choosePosition(index){showSwap({index});}
function showSwap({index=null,incoming=null}={}){
 const m=state.match;if(m.status==='ended')return;
 if(incoming&&!bench().some(p=>p.id===incoming))return;
 const labels=positionNames(m),times=minutes(m),selected=index!==null?m.lineup[index]:incoming;
 const time=id=>`<small class="swap-time" data-time="${esc(id)}">${clock(times[id]||0)}</small>`;
 const field=m.lineup.map((id,i)=>{
  const disabled=index!==null&&(i===index||!id);
  return `<button class="position pos-${i} ${i===index?'selected':''}" data-action="assign" data-index="${index===null?i:index}" data-id="${esc(incoming||id||'')}" ${disabled?'disabled':''} aria-label="${labels[i]}: ${esc(name(id))}"><span class="shirt">${id?esc(initials(name(id))):'+'}</span><span class="position-name">${labels[i]}</span><span class="player-name">${esc(id?name(id):'Vrije plek')}</span>${time(id)}</button>`;
 }).join('');
 const bank=bench().map(p=>`<button class="${p.id===incoming?'selected':''}" data-action="${index===null?'bench':'assign'}" ${index===null?'':`data-index="${index}" `}data-id="${esc(p.id)}" aria-pressed="${p.id===incoming}"><strong>${esc(p.name)}</strong>${time(p.id)}</button>`).join('');
 modal(incoming?`${esc(name(incoming))} inbrengen`:`${labels[index]} wisselen`, `<p class="hint swap-help">${incoming?'Tik op de veldspeler die naar de bank gaat.':`Kies een grijze bankspeler voor ${esc(selected?name(selected):'deze positie')}, of tik op een veldspeler om van plek te ruilen.`} Tijd = totaal gespeeld deze wedstrijd.</p><div class="pitch swap-pitch formation-${m.formation}">${field}</div><h3 class="choice-heading">Wisselbank</h3><div class="bench swap-bench">${bank||'<p class="hint">Geen bankspelers beschikbaar.</p>'}</div>${index!==null&&selected?`<button class="wide text-button" data-action="assign" data-index="${index}" data-id="">Naar de bank · positie leegmaken</button>`:''}`);
 tick();
}
function assign(index,id){const m=state.match;if(m.status==='ended')return;if(id&&!present().some(p=>p.id===id))throw Error('Deze speler is niet aanwezig.');const line=[...m.lineup];if(line[index]===id){dialog.close();return;}const oldIndex=line.indexOf(id);if(id&&oldIndex!==-1)line[oldIndex]=line[index];line[index]=id||null;changeGame(state,m.status==='ready'?'opstelling':'wissel',()=>setLineup(m,line));dialog.close();update();toast(m.status==='ready'?'Opstelling opgeslagen':'Wissel opgeslagen');}
...
const actions={
 ...
 'bench':b=>showSwap({incoming:b.dataset.id}),
 ...
 'confirm-half':()=>{if(state.match.status!=='live'||state.match.half!==1)return;pause(state.match);changeGame(state,'tweede helft',m=>{m.half=2;m.halfStartedAt=elapsed(m);});keepAwake();dialog.close();update();if(needsKeeperChange(state.match))showSuggestions(true);},
 ...
 'confirm-finish':()=>{const m=state.match;if(m.status!=='live')return;pause(m);m.status='ended';state.undoHistory=[];state.history.push({match:structuredClone(m),players:state.players.map(p=>({id:p.id,name:p.name,present:p.present}))});keepAwake();dialog.close();update();toast('Wedstrijd bewaard');},
 ...
 'edit-player':b=>{const p=state.players.find(p=>p.id===b.dataset.id);modal('Speler bewerken',`<form id="player-form" data-id="${esc(p.id)}" class="stack"><label>Voornaam<input name="name" value="${esc(p.name)}" maxlength="40" required></label><label>Voorkeurslinie<select name="preferredLine">${Object.entries(PREFERRED_LINES).map(([key,label])=>`<option value="${key}" ${(p.preferredLine||'any')===key?'selected':''}>${label}</option>`).join('')}</select></label><p class="hint">Een voorkeur, geen vaste plek. Speeltijd en afwisseling blijven leidend.</p><button type="submit" class="primary">Opslaan</button></form>${state.match.status==='ready'?`<button class="text-button danger" data-action="delete-player" data-id="${esc(p.id)}">Speler verwijderen</button>`:''}`);},
...
function playingStatsCard(){
 const stats=playingStatistics(state);
 return `<details class="card"><summary>Totale speeltijd</summary><p class="hint">Afgeronde wedstrijden · min:sec, inclusief keepen. Minste totale speeltijd eerst. Gemiddeld per aanwezige wedstrijd; de lopende wedstrijd telt hier nog niet mee.</p>${stats.map(p=>`<div class="player-row"><div class="player-info"><strong>${esc(p.name)}</strong><small>${p.matches} wedstrijden · Gemiddeld ${clock(p.average)}<br>Laatste bewaarde wedstrijd: ${p.last===null?'—':clock(p.last)}</small></div><strong class="player-time">${clock(p.total)}</strong></div>`).join('')||'<p class="muted small">Nog geen spelers of wedstrijden.</p>'}<p class="hint">Bij oudere wedstrijden zonder aanwezigheidsregistratie telt voor het gemiddelde alleen aantoonbaar gespeelde tijd. Een streepje betekent: geen deelname vastgelegd in de laatste wedstrijd.</p></details>`;
}
function historyPlayingTimes(h){
 const m=h.match,times=minutes(m),used=new Set([...m.initialLineup,...m.lineup,...m.events.filter(e=>e.type==='lineup').flatMap(e=>e.lineup)]);
 return `<details open class="history-minutes"><summary>Speeltijd deze wedstrijd</summary><p class="hint">Minuten:seconden, inclusief keepen.</p>${h.players.map(p=>`<div class="history-row"><span>${esc(p.name)}</span><strong>${used.has(p.id)||times[p.id]||p.present===true?clock(times[p.id]||0):p.present===false?'Afwezig':'Niet vastgelegd'}</strong></div>`).join('')||'<p>Geen spelers geregistreerd.</p>'}</details>`;
}
let substitutionDraft=null;
function showSuggestions(keeper=false){
 if(state.match.status!=='live')return;
 substitutionDraft=proposeSubstitutions(state,{keeper});const p=substitutionDraft,m=state.match,times=minutes(m),labels=positionNames(m);
 if(!p.changes.length){modal(keeper?'Keeper tweede helft':'Wisselvoorstel',`<p>${keeper?'Er is geen andere aanwezige speler om te keepen.':'Nog geen wisselvoorstel beschikbaar. Spelers staan eerst minimaal één speelminuut op het veld of op de bank om direct terugwisselen te voorkomen.'}</p><p class="hint">Handmatig wisselen kan altijd via de wisselbank of Opstelling.</p><button class="wide" data-action="close">Sluiten</button>`);return;}
 const incoming=keeper?present().filter(x=>x.id!==m.lineup[0]):bench();
 modal(keeper?'Keeper voor de tweede helft':'Wisselvoorstel',`<p class="muted small">${keeper?'Voorstel voor de hele tweede helft. Minste speeltijd eerst. Kies je iemand uit het veld, dan ruilt die van plek met de keeper.':'Minste speeltijd erin, meeste speeltijd eruit. Bij vergelijkbare minuten wegen voorkeurslinie en afwisseling mee. De keeper blijft staan.'}</p><form id="substitution-form" class="stack">${p.changes.map((c,i)=>`<fieldset class="suggestion"><legend>${keeper?'Keeperwissel':'Wissel '+(i+1)}</legend><label>${keeper?'Nieuwe keeper':'Erin'}<select name="in${i}">${!keeper?'<option value="">Deze wissel overslaan</option>':''}${incoming.map(x=>`<option value="${esc(x.id)}" ${x.id===c.inId?'selected':''}>${esc(x.name)} · ${clock(times[x.id]||0)}${x.preferredLine&&x.preferredLine!=='any'?' · '+PREFERRED_LINES[x.preferredLine]:''}</option>`).join('')}</select></label>${keeper?`<input type="hidden" name="position${i}" value="0"><p class="hint">Voor ${esc(name(c.outId))} · ${clock(times[c.outId]||0)} gespeeld</p>`:`<label>Eruit / positie<select name="position${i}">${m.lineup.map((id,index)=>index?`<option value="${index}" ${index===c.index?'selected':''}>${esc(name(id))} · ${clock(times[id]||0)} · ${labels[index]}</option>`:'').join('')}</select></label>`}</fieldset>`).join('')}<p id="proposal-error" class="danger" role="alert"></p><button class="primary" type="submit">${keeper?'Keeperwissel bevestigen':'Wissels bevestigen'}</button><button type="button" data-action="close">Annuleren</button></form><p class="hint">Minuten bij het openen van dit voorstel. Je kunt de keuzes aanpassen. Een bevestigde wissel kun je herstellen.</p>`);
}
function confirmDeleteHistory(b){
 const h=state.history.find(h=>h.match.id===b.dataset.id);if(!h)return;
 const m=h.match,s=score(m),current=m.id===state.match.id;
...
Object.assign(actions,{
 'suggest-subs':()=>showSuggestions(false),
 'suggest-keeper':()=>showSuggestions(true),
 'history':showHistory,
 'delete-history':confirmDeleteHistory,
 'confirm-delete-history':b=>{if(!deleteHistory(state,b.dataset.id))return;dialog.close();update();toast('Wedstrijd verwijderd');},
...
document.addEventListener('submit',e=>{
 if(e.target.id!=='substitution-form')return;e.preventDefault();
 if(!substitutionDraft)return;
 const data=new FormData(e.target),p=structuredClone(substitutionDraft);
 p.changes=p.changes.map((c,i)=>{const index=Number(data.get('position'+i));return {index,inId:String(data.get('in'+i)||''),outId:state.match.lineup[index]};}).filter(c=>c.inId);
 try{applySubstitutionProposal(state,p);substitutionDraft=null;dialog.close();update();toast('Wissel opgeslagen. Vergist? Gebruik Herstel.');}
