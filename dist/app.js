...
import {freshState,freshMatch,uid,elapsed,score,minutes,start,pause,setLineup,setFormation,positionNames,FORMATIONS,normalizeTeamUrl,undo,validateState,removeGoal,matchSummary,changeGame,undoGame,reminders,backupDue,topScorers,deleteHistory} from './model.js';
...
function matchView(){
 ...
 return `<div class="single match-screen">
  ...
  <section class="card scoreboard">
   ...
   <div class="score-line">${team('us','Nieuwerkerk JO10-8')}<span class="separator">:</span>${team('them',m.opponent||'Tegenstander')}</div>
...
function showSuggestions(keeper=false){
 ...
 if(!p.changes.length){modal(keeper?'Keeper tweede helft':'Wisselvoorstel',`<p>${keeper?'Er is geen andere aanwezige speler om te keepen.':'Nog geen wissel met duidelijk speeltijdvoordeel. Een voorstel verschijnt zodra een bankspeler minstens één minuut minder heeft gespeeld dan een veldspeler.'}</p><p class="hint">Handmatig wisselen kan altijd via de wisselbank of Opstelling.</p><button class="wide" data-action="close">Sluiten</button>`);return;}
